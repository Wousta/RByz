...
// This class and parent class are a crime against programming principles, Some
// day it will be refactored into smaller classes

/**
 * @brief RegisteredMnistTrain class for handling registered MNIST dataset
 * training.
 *
...
private:
  ...
  size_t &forward_pass_indices_mem_size =
      forward_pass_info.forward_pass_indices_mem_size;
  ...
  inline char *getBasePointerForIndex(size_t image_idx) const {
    return static_cast<char *>(reg_data) + (image_idx * data_info.get_sample_size());
  }

public:
  RegisteredMnistTrain(int worker_id, int num_workers, int64_t subset_size,
                       bool dataset_type = MNIST);
  ~RegisteredMnistTrain();

  std::vector<torch::Tensor>
  runMnistTrain(int round, const std::vector<torch::Tensor> &w) override;
  ...
  size_t getSampleSize() { return data_info.get_sample_size(); }
  ...
  size_t getForwardPassIndicesMemSize() {
    return forward_pass_indices_mem_size;
  }
