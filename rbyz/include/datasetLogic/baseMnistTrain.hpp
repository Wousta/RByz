...
public:
  ...
  torch::Tensor forward(torch::Tensor x) {
    ...
    x = torch::dropout(
        x, /*p=*/0.0,
        /*training=*/is_training()); // Dropout probability set to 0 for now
                                     // during testing (was 0.5)
    x = fc2->forward(x);
...
protected:
  ...
  void test(oldNet &model, torch::Device device, DataLoader &data_loader,
            size_t dataset_size);

  torch::Device init_device();
  virtual SubsetSampler get_subset_sampler(
...
public:
  ...
  runMnistTrain(int round, const std::vector<torch::Tensor> &w) = 0;
  virtual void runInference(const std::vector<torch::Tensor> &w) = 0;

  virtual std::vector<torch::Tensor> getInitialWeights();
  void saveModelState(const std::vector<torch::Tensor> &w,
  ...
  void testModel() { test(model, device, *test_loader, test_dataset_size); }
  ...
  void setTrainAccuracy(float new_train_accuracy) {
    train_accuracy = new_train_accuracy;
  }
  float getTestAccuracy() { return test_accuracy; }
  void setTestAccuracy(float new_test_accuracy) {
