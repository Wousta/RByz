...
void RByzSrvr::renewTrustedClientsColumn(DatasetSplitter &splitter) {
  ...
  for (auto &client : clnt_data_vec) {
    ...
    rdma_ops.exec_rdma_read(rdma_size, local_info, remote_info, client.index, RdmaOps::Mode::async);
...
void RByzSrvr::updateTS(ClientDataRbyz &clnt_data, float srvr_loss, float srvr_error_rate) {
...
torch::Tensor RByzSrvr::aggregate_updates(const std::vector<torch::Tensor> &client_updates,
...
void RByzSrvr::writeServerVD(DatasetSplitter &splitter, float proportion) {
  ...
  for (int i = 0; i < clnt_data_vec.size(); i++) {
    ...
    for (size_t j = 0; j < clnt_chunks.size(); j++) {
      ...
      rdma_ops.exec_rdma_write(splitter.getChunkSize(), local_info, remote_info, i,
                               RdmaOps::Mode::async);
...
bool RByzSrvr::processVDOut(ClientDataRbyz &clnt_data, bool check_byz) {
  ...
  if (inserted_indices_set.size() != clnt_data.inserted_indices.size()) {
    Logger::instance().log(
        "Warning: Inserted indices set size does "
        "not match the number of inserted indices " +
        std::to_string(clnt_data.inserted_indices.size()) + " vs " +
  ...
  for (size_t i = 0; i < clnt_num_samples && validation_passed; ++i) {
    ...
    if (inserted_indices_set.find(clnt_idx) != inserted_indices_set.end()) {
      ...
      if (processed_samples < 1) {
        Logger::instance().log(
            "    > Processing sample of client" + std::to_string(clnt_data.index) + ": idx " +
  ...
  float processed_samples_proportion = static_cast<float>(processed_samples) / clnt_num_samples;
  ...
  Logger::instance().log("    $ VD validation passed for a proportion of " +
                         ...
                         "%' inserted samples" + "for client " + std::to_string(clnt_data.index) +
                         " ·····$\n");
...
void RByzSrvr::initTimeoutTime() {
  for (ClientDataRbyz &clnt_data : clnt_data_vec) {
    ...
    Logger::instance().log(
        ...
        " initial limit step time: " + std::to_string(clnt_data.limit_step_time.count()) + " ms\n");
  }
}
  ...
  while (!timed_out && clnt_data.local_step < clnt_data.next_step && clnt_data.round <= round) {
    ...
    if (total_time_waited > clnt_data.limit_step_time.count() * TIMEOUT_SLACK) {
      ...
      if (clnt_data.steps_to_finish <= timeouts_.min_steps) {
        ...
        long int new_limit =
            static_cast<long int>(std::ceil(clnt_data.limit_step_time.count() * TIMEOUT_MULT));
        clnt_data.limit_step_time = millis(new_limit);
        clnt_data.steps_to_finish = timeouts_.step_range(rng);

        int new_mid = static_cast<int>(std::floor(clnt_data.steps_to_finish * 0.75));
        timeouts_.mid_steps = std::max(new_mid, timeouts_.min_steps);
        timeouts_.step_range =
            std::uniform_int_distribution<int>(timeouts_.mid_steps, clnt_data.steps_to_finish);
        Logger::instance().log(
            "    -> Client " + std::to_string(clnt_idx) +
      ...
      Logger::instance().log("    -> Client " + std::to_string(clnt_idx) + " timed out\n");
      timed_out = true;
    }
...
void RByzSrvr::vdColAttack(float proportion) {
  ...
  if (mngr.n_clients % 2 == 0) minority -= 1;

  Logger::instance().log("    -> Corrupting " + std::to_string(minority) + " extra columns\n");
  ...
  Logger::instance().log("    -> Corrupted " + std::to_string(images_corrupted) + " images\n");
}

//////////////////////////////////////////////////////////////
/////////////////////// RBYZ ALGORITHM ///////////////////////
void RByzSrvr::runRByzServer(int n_clients, std::vector<torch::Tensor> &w, RegMemSrvr &regMem) {
  Logger::instance().logCustom(t_params.logs_dir, t_params.clnts_renew_file,
  ...
  for (int round = 1; round <= global_rounds; round++) {
    Logger::instance().log("\n\n=====  ROUND " + std::to_string(round) + " STARTED  =====n");
    ...
    for (ClientDataRbyz &client : clnt_data_vec) {
      ...
      rdma_ops.exec_rdma_read(client.forward_pass_mem_size, CLNT_FORWARD_PASS_IDX, client.index);
      rdma_ops.exec_rdma_read(client.forward_pass_indices_mem_size, CLNT_FORWARD_PASS_INDICES_IDX,
                              client.index);
    ...
    Logger::instance().log(" Trust Scores at end of round " + std::to_string(round) + ":\n");
    for (const ClientDataRbyz &clnt_data : clnt_data_vec) {
      Logger::instance().log("    Client " + std::to_string(clnt_data.index) +
    ...
    for (ClientDataRbyz &client : clnt_data_vec) {
      ...
      while (regMem.clnt_ready_flags[client.index] != round && !timed_out) {
        ...
        if (initial_time > limit_step_time) {
          timed_out = true;
          Logger::instance().log("    -> Timeout client " + std::to_string(client.index) + "\n");
        }
      }

      if (total_time_waited > 0)
        Logger::instance().log("    -> Server waited: " + std::to_string(total_time_waited) +
      ...
      if (!timed_out && client.trust_score != 0 && client.include_in_agg) {
        ...
        torch::Tensor flat_tensor =
            torch::from_blob(regMem.clnt_ws[client.index], {static_cast<long>(numel_server)},
                             torch::kFloat32)
                .clone();
    ...
    Logger::instance().logCustom(
        ...
        std::to_string(total_steps) + " " + std::to_string(mngr.test_accuracy) + "\n");

    std::cout << "///////////////// Round " << round << " completed /////////////////\n";
    Logger::instance().log("\n////// Round " + std::to_string(round) + " completed //////n");
  }
