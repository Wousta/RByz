...
torch::data::Example<> RegCIFAR10::get(size_t original_index) {
  ...
  // uint8_t* img_ptr = reinterpret_cast<uint8_t*>(reinterpret_cast<uint8_t*>(sample) + index_size +
  // label_size);
  float *img_ptr =
      reinterpret_cast<float *>(reinterpret_cast<uint8_t *>(sample) + index_size + label_size);

  torch::Tensor image =
      torch::from_blob(img_ptr, {3, kImageRows, kImageColumns},  // [channels, height, width]
                       options)
          .clone();
...
bool RegCIFAR10::is_train() const noexcept {
