...
class RegisteredMNIST : public torch::data::Dataset<RegisteredMNIST> {
 ...
 public:
  ...
  inline torch::data::Example<> get(size_t original_index) override {
    ...
    float* img_ptr =
        reinterpret_cast<float*>(reinterpret_cast<uint8_t*>(sample) + index_size + label_size);
    torch::Tensor image =
        torch::from_blob(img_ptr, {1, 28, 28},  // MNIST image dimensions [channels, height, width]
                         options)
            .clone();
  ...
  std::optional<size_t> size() const override { return num_samples; }
};