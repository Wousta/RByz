 ...
 */
void prepareRdmaRegistration(int n_clients, TrainInputParams &t_params, std::vector<RegInfo> &reg_info,
                             RegMemSrvr &regMem, std::vector<ClientDataRbyz> &clnt_data_vec, IRegDatasetMngr &mngr) {
...
void allocateServerMemory(int n_clients, TrainInputParams &t_params, RegMemSrvr &regMem,
                          std::vector<ClientDataRbyz> &clnt_data_vec, IRegDatasetMngr &mngr) {
  size_t sample_size = mngr.data_info.get_sample_size();
  std::vector<size_t> clnts_samples_count =
      mngr.getClientsSamplesCount(regMem.clnt_subset_size, regMem.srvr_subset_size, regMem.dataset_size);
  ...
  for (int i = 0; i < n_clients; i++) {
    ...
    size_t forward_pass_mem_size = samples_fpass * values_per_sample * bytes_per_value;
...
int main(int argc, char *argv[]) {
  ...
  latency->reserve(10);
  auto cli =
      lyra::cli() | lyra::opt(srvr_ip, "srvr_ip")["-i"]["--srvr_ip"]("srvr_ip").required() |
      ...
      lyra::opt(t_params.vd_prop_write, "vd_prop_write")["--vd_prop_write"]("Proportion of chunks writable on client") |
      lyra::opt(t_params.test_renewal_freq, "test_renewal_freq")["--test_renewal_freq"]("Frequency of test renewal") |
      lyra::opt(t_params.overwrite_poisoned, "overwrite_poisoned")["--overwrite_poisoned"]("VD samples overwrite") |
      lyra::opt(t_params.wait_all, "wait_all")["--wait_all"]("Wait for all clients to finish") |
      lyra::opt(t_params.batches_fpass_prop, "batches_fpass")["--batches_fpass"]("Num of batches for forward pass") |
      lyra::opt(t_params.srvr_wait_inc, "srvr_wait_inc")["--srvr_wait_inc"]("For timeouts experiment") |
      lyra::opt(t_params.extra_col_poison_prop, "extra_col_poison_prop")["--extra_col_poison_prop"](
          "Proportion of extra columns to poison in the dataset");
  ...
  prepareRdmaRegistration(n_clients, t_params, reg_info, *regMem, clnt_data_vec, *reg_mngr);
