 ...
 */
void prepareRdmaRegistration(int n_clients, TrainInputParams &t_params,
                             std::vector<RegInfo> &reg_info, RegMemSrvr &regMem,
...
torch::Tensor
aggregateUpdatesAvg(const std::vector<torch::Tensor> &client_updates,
  ...
  aggregated_update /=
      static_cast<float>(client_updates.size() + 1); // +1 for server update
  // aggregated_update = torch::clamp(aggregated_update, -1.0f, 1.0f); // Clamp
  // values to [-1, 1]
  return aggregated_update;
}

torch::Tensor
aggregateUpdates(const std::vector<torch::Tensor> &client_updates,
  ...
  for (int i = 0; i < client_updates.size(); i++) {
    ...
    float client_full_norm =
        torch::norm(client_full_updates[i], 2).item<float>();
...
run_fltrust_srvr(int n_clients, TrainInputParams t_params,
                 IRegDatasetMngr &mngr, RegMemSrvr &regMem,
  ...
  for (int round = 1; round <= t_params.global_iters_fl; round++) {
    ...
    for (size_t i = 0; i < polled_clients.size(); i++) {
      ...
      while (regMem.clnt_ready_flags[client] != round && !timed_out) {
        ...
        if (initial_time.count() % 5000 == 0) {
          Logger::instance().log(
              "     Client " + std::to_string(client) + " ready flag: " +
              std::to_string(regMem.clnt_ready_flags[client]) +
        ...
        if (initial_time > limit_step_time) {
          ...
          Logger::instance().log(
              ...
              std::to_string(client) + " for round " + std::to_string(round) +
              "\n");
        }
      }
      Logger::instance().log(
          "    -> Server waited: " + std::to_string(total_wait_time) +
          " ms for client " + std::to_string(client) + " ready flag: " +
          std::to_string(regMem.clnt_ready_flags[client]) + "\n");
    ...
    torch::Tensor aggregated_update =
        aggregateUpdates(clnt_updates, clnt_full_updates, srvr_update,
                         srvr_full_update, clnt_indices, log_TS_vec, t_params);
 ...
 */
void allocateServerMemory(int n_clients, TrainInputParams &t_params,
                          RegMemSrvr &regMem,
                          std::vector<ClientDataRbyz> &clnt_data_vec,
                          IRegDatasetMngr &mngr) {
  size_t sample_size = mngr.data_info.get_sample_size();
  std::vector<size_t> clnts_samples_count = mngr.getClientsSamplesCount(
      regMem.clnt_subset_size, regMem.srvr_subset_size, regMem.dataset_size);
  ...
  for (int i = 0; i < n_clients; i++) {
    ...
    if (t_params.batches_fpass_prop > 0) {
      samples_fpass =
          std::min(samples_fpass, batches_fpass * mngr.kTrainBatchSize);
...
int main(int argc, char *argv[]) {
  ...
  auto cli =
      lyra::cli() |
      lyra::opt(srvr_ip, "srvr_ip")["-i"]["--srvr_ip"]("srvr_ip").required() |
      lyra::opt(port, "port")["-p"]["--port"]("port").required() |
      lyra::opt(n_clients, "n_clients")["-w"]["--n_clients"]("n_clients").required() |
      ...
      lyra::opt(t_params.local_learn_rate, "local_learn_rate")["--local_learn_rate"]("local learning rate") |
      ...
      lyra::opt(t_params.label_flip_type, "label_flip_type")["--label_flip_type"]("Label flip type") |
      ...
      lyra::opt(t_params.vd_prop_write, "vd_prop_write")["--vd_prop_write"]("Proportion of total chunks writable on client") |
      ...
      lyra::opt(t_params.wait_all, "wait_all")["--wait_all"]("Wait for all clients to finish") |
  ...
  regMem = std::make_unique<RegMemSrvr>(n_clients, reg_sz_data,
                                        reg_mngr->data_info.reg_data);
  ...
  Logger::instance().logCustom(dir, t_params.ts_file,
                               std::to_string(rounds) + "\n");
  Logger::instance().logCustom(dir, t_params.ts_file,
                               std::to_string(n_clients) + "\n");
  Logger::instance().logCustom(dir, t_params.ts_file,
                               std::to_string(t_params.n_byz_clnts) + "\n");
  ...
  std::vector<torch::Tensor> w =
      run_fltrust_srvr(n_clients, t_params, *reg_mngr, *regMem, clnt_data_vec);
  ...
  Logger::instance().log("Total time taken: " + std::to_string(elapsed) +
                         " seconds\n");

  reg_mngr->runTesting();

  std::string acc_msg =
      "Accuracy " + std::to_string(reg_mngr->test_accuracy) + "\n";
  std::string time_msg = "Time " + std::to_string(elapsed) + "\n";
  std::string vd_msg = "clnt_vd_proportion " +
                       std::to_string(t_params.clnt_vd_proportion) + "\n";
  std::string vd_prop_msg =
      "vd_prop_write " + std::to_string(t_params.vd_prop_write) + "\n";
  std::string test_renew_msg =
      "test_renewal_freq " + std::to_string(t_params.test_renewal_freq) + "\n";
  std::string recall_msg = "src_targ_class " +
                           std::to_string(reg_mngr->src_class) + " " +
                           std::to_string(reg_mngr->target_class) + "\n";
  std::string miss_samples_msg = "missclassed_samples " +
                                 std::to_string(reg_mngr->missclassed_samples) +
                                 "\n";
  std::string class_recall_msg =
      "src_class_recall " + std::to_string(reg_mngr->src_class_recall) + "\n";
  std::string rounds_to_converge_msg =
      "rounds_to_converge " + std::to_string(rbyz_aux.getRoundsToConverge()) +
      "\n";
  ...
  Logger::instance().logCustom(dir, t_params.acc_file,
                               "$ END OF EXECUTION $\n");
  Logger::instance().logCustom(dir, t_params.ts_file, "$ END OF EXECUTION $\n");
  Logger::instance().logCustom(dir, t_params.included_agg_file,
                               "$ END OF EXECUTION $\n");
  Logger::instance().logCustom(dir, t_params.clnts_renew_file,
                               "$ END OF EXECUTION $\n");
