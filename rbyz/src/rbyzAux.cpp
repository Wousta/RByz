...
void RByzAux::renewTrustedClientsColumn(
    RegMnistSplitter &splitter, std::vector<ClientDataRbyz> &clnt_data_vec) {
...
void RByzAux::updateTS(std::vector<ClientDataRbyz> &clnt_data_vec,
  ...
  // Logger::instance().log("        -> Minimum loss: " +
  // std::to_string(min_w_loss) +
  ...
  // Logger::instance().log("        -> Server loss: " +
  // std::to_string(srvr_loss) +
  //                        ", Server error rate: " +
  //                        std::to_string(srvr_error_rate) + "\n");
  // Logger::instance().log("        -> Max loss: " + std::to_string(max_loss) +
  //                        ", Max error rate: " + std::to_string(max_err) +
  //                        "\n");
  // Logger::instance().log("        -> Calculated loss_Calc: " +
  // std::to_string(loss_Calc) +
...
RByzAux::aggregate_updates(const std::vector<torch::Tensor> &client_updates,
  ...
  Logger::instance().log(
      "Final Aggregated update: " +
      aggregated_update
          .slice(0, 0, std::min<size_t>(aggregated_update.numel(), 5))
          .toString() +
      "\n");
  Logger::instance().log("================================\n");
...
void RByzAux::runRByzServer(int n_clients, std::vector<torch::Tensor> &w,
  ...
  for (int round = 1; round <= global_rounds; round++) {
    ...
    if (clnt_updates.empty()) {
      ...
      aggregated_update =
          aggregate_updates(clnt_updates, clnt_data_vec, clnt_indices);
    }
