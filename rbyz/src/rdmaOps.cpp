...
int RdmaOps::exec_rdma_read(uint32_t size, uint32_t loc_info_idx, uint32_t rem_info_idx, int conn_data_idx) {
  ...
  return norm::read(this->conn_data[conn_data_idx], {size}, {local_info}, netflags_sync, remote_info, this->latency, posted_wqes[conn_data_idx]);
}
...
int RdmaOps::exec_rdma_write(uint32_t size, uint32_t loc_info_idx, uint32_t rem_info_idx, int conn_data_idx) {
  ...
  RemoteInfo remote_info;
  remote_info.indx = rem_info_idx;
  return norm::write(this->conn_data[conn_data_idx], {size}, {local_info}, netflags_sync, remote_info, this->latency, posted_wqes[conn_data_idx]);
}
...
int RdmaOps::exec_rdma_write(uint32_t size, LocalInfo &local_info, RemoteInfo &remote_info, int conn_data_idx, bool is_sync) {
  if (is_sync) {
    ...
    return norm::write(this->conn_data[conn_data_idx], {size}, {local_info}, netflags_no_sync, remote_info, this->latency, posted_wqes[conn_data_idx]);
  }
}
...
int RdmaOps::exec_rdma_CAS(uint32_t size, uint32_t loc_info_idx, uint32_t rem_info_idx, uint64_t compare_add, uint64_t swap, int conn_data_idx) {
  ...
  RemoteInfo remote_info;
  remote_info.indx = rem_info_idx;
  return norm::CAS(this->conn_data[conn_data_idx], {size}, {local_info}, netflags_sync, remote_info, this->latency, posted_wqes[conn_data_idx]);
}
